// Πληροφορίες από πραγματικές παραστάσεις
const shows = {
  "Don Giovanni": {
    name: "Ντον Τζοβάννι",
    composer: "Βόλφγκανγκ Αμαντέους Μότσαρτ",
    libretto: "Λορένζο ντα Πόντε",
    production:
      "Συμπαραγωγή με την Όπερα του Γκαίτεμποργκ (Σουηδία) και τη Βασιλική Όπερα της Δανίας",
    description: `Η νέα παραγωγή του Ντον Τζοβάννι του Μότσαρτ αποτελεί μια μεγάλη διεθνή συμπαραγωγή της Εθνικής Λυρικής Σκηνής με δύο σημαντικά βορειοευρωπαϊκά λυρικά θέατρα, την Όπερα του Γκαίτεμποργκ και τη Βασιλική Όπερα της Δανίας. Τη σκηνοθεσία υπογράφει ο διακεκριμένος σκηνοθέτης και καλλιτεχνικός διευθυντής της Βασιλικής Όπερας της Δανίας Τζων Φούλτζεϊμς. Η παραγωγή, μετά την αθηναϊκή πρεμιέρα του Δεκεμβρίου του 2021, θα παρουσιαστεί σε Γκαίτεμποργκ και Κοπεγχάγη. O Φούλτζεϊμς τοποθετεί τη δράση της όπερας σε ένα σημερινό ξενοδοχείο, έναν μικρόκοσμο μιας πόλης, έναν τόπο συνάντησης ανθρώπων, όπου το ιδιωτικό γίνεται δημόσιο και ο χρόνος παραμονής είναι περιορισμένος. Ο σκηνοθέτης σημειώνει: «Ο Ντον Τζοβάννι είναι μια όπερα που διαρκεί στον χρόνο επειδή είναι πολύπλευρη· έχει πολύ χιούμορ και σπουδαία θεατρικά σκηνικά, όλα ενταγμένα μέσα σε μια σκοτεινή ιστορία μυστηρίου για έναν αποπλανητή που κάποιος πρέπει να τον σταματήσει. Είναι ένα έργο για το πώς ζούμε μαζί στην κοινωνία και, όπως συμβαίνει πάντα με την όπερα, για το πώς αντιμετωπίζουμε τον θάνατο. Η κορύφωσή του έγκειται στον πολυαναμενόμενο θάνατο του ίδιου του Ντον Τζοβάννι, καθώς προτιμάει τον θάνατο, ακόμα και την κόλαση, από το να απαρνηθεί το ποιος είναι. Ο θάνατός του είναι μια έκφραση της τάξης που επανεπιβεβαιώνει την ισχύ της έναντι της αχαλίνωτης ελευθερίας που εκείνος αντιπροσωπεύει». Στον Ντον Τζοβάννι –ένα από τα ορόσημα του οπερατικού ρεπερτορίου– ο Μότσαρτ αποδίδει με τη μουσική του όλες τις όψεις της πορείας του ανθρώπου που τα βάζει με τον Θεό και παίρνει την τύχη του στα χέρια του.`,
    dates: [
      "2021-12-12",
      "2021-12-13",
      "2021-12-16",
      "2021-12-18",
      "2021-12-20",
      "2021-12-22",
      "2021-12-27",
      "2021-12-29",
    ],
    crew: {
      conductor: "Ντάνιελ Σμιθ",
      director: "Τζων Φούλτζεϊμς",
      "associate director": "Αϋλίν Μπόζοκ",
      sets: "Ντικ Μπερντ",
      costumes: "Άνμαρι Γουντς",
      choreographer: "Μαξίν Μπρέιαμ",
      "lighting designer": "Φαμπιάνα Πιτσόλι",
      "video designer": "Ουίλ Ντιούκ",
      "programmer video": "Νταν Τρέντσαρντ",
      "chorus master": "Αγαθάγγελος Γεωργακάτος",
    },
    roles: {
      "Ντον Τζοβάννι": {
        "Τάσης Χριστογιαννόπουλος": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Διονύσης Σούρμπης": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
      "Ντόννα Άννα": {
        "Βασιλική Καραγιάννη": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Σιρανούς Τσαλικιάν": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
      "Ντον Οττάβιο": {
        "Γιάννης Χριστόπουλος ": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Βασίλης Καβάγιας": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
      Διοικητής: {
        "Πέτρος Μαγουλάς": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Αλέξανδρος Σταυρακάκης": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
      "Ντόννα Ελβίρα": {
        "Άννα Στυλιανάκη": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Άρτεμις Μπόγρη": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
      Λεπορέλλο: {
        "Τάσος Αποστόλου": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Χάρης Ανδριανός": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
      Μαζέττο: {
        "Νίκος Κοτενίδης": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Γιώργος Ματθαιακάκης": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
      Tσερλίνα: {
        "Χρύσα Μαλιαμάνη": [
          "2021-12-12",
          "2021-12-16",
          "2021-12-20",
          "2021-12-27",
        ],
        "Μιράντα Μακρυνιώτη": [
          "2021-12-13",
          "2021-12-18",
          "2021-12-22",
          "2021-12-29",
        ],
      },
    },
  },

  "The Marriage of Figaro": {
    name: "Οι Γάμοι του Φίγκαρο",
    composer: "Βόλφγκανγκ Αμαντέους Μότσαρτ",
    libretto: "Λορένζο ντα Πόντε",
    production: "Εθνική Λυρική Σκηνή",
    description: ` Έχουν περάσει τρία χρόνια από τότε που ο Φίγκαρο κατάφερε να παντρέψει τον Κόμη Αλμαβίβα με τη Ροζίνα και τώρα είναι η σειρά του να παντρευτεί τη Σουζάννα, μέλος του υπηρετικού τους προσωπικού, όπως και ο ίδιος. Ωστόσο ο Κόμης έχει αρχίσει να βαριέται τον έγγαμο βίο και προσπαθεί να εκμεταλλευτεί το προνόμιο της «πρώτης νύχτας» σύμφωνα με το οποίο ένας άρχοντας μπορεί να κοιμηθεί με μια υπηρέτριά του την πρώτη νύχτα του γάμου της. Οι μηχανορραφίες του Κόμη όμως αποκαλύπτονται, ο Φίγκαρο με τη Σουζάννα παντρεύονται και ο ίδιος ο Κόμης υποχρεώνεται να ζητήσει συγχώρεση από τη σύζυγό του για την ανάρμοστη αυτή συμπεριφορά, δίνοντας στο έργο ένα αίσιο τέλος. `,
    dates: ["2020-5-13", "2021-12-15", "2021-12-17"],

    crew: {
      conductor: "Βασίλης Χριστόπουλος",
      director: "Αλέξανδρος Ευκλείδης",
      dramaturgy: "Αγγέλα-Κλεοπάτρα Σαρόγλου",
      sets: "Γιάννης Κατρανίτσας",
      costumes: "Ιωάννα Τσάμη",
      "lighting designer": "Μελίνα Μάσχα",
      "chorus master": "Αγαθάγγελος Γεωργακάτος",
    },
    roles: {
      "Κόμης Αλμαβίβα": {
        "Δημήτρης Πλατανιάς": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      "Κόμησσα Αλμαβίβα": {
        "Τσέλια Κοστέα": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Σουζάννα: {
        "Αφροδίτη Πατουλίδου": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Φίγκαρο: {
        "Διονύσης Σούρμπης": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Κερουμπίνο: {
        "Μιράντα Μακρυνιώτη": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Μαρτσελλίνα: {
        "Μαρισία Παπαλεξίου ": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Μπάρτολο: {
        "Γιάννης Γιαννίσης": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Μπαζίλιο: {
        "Χρήστος Κεχρής": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Μπαζίλιο: {
        "Χρήστος Κεχρής": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      "Ντον Κούρτσιο": {
        "Γιάννης Καλύβας": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Μπαρμπαρίνα: {
        "Μαριλένα Στριφτόμπολα": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
      Αντόνιο: {
        "Κωστής Ρασιδάκις": ["2020-5-13", "2021-12-15", "2021-12-17"],
      },
    },
  },

  "Madame Butterfly": {
    name: "Μαντάμα Μπαττερφλάι",
    composer: "Τζάκομο Πουτσίνι",
    libretto: "Λουίτζι Ίλικα και Τζουζέπε Τζακόζα",
    production: "Εθνική Λυρική Σκηνή",
    description: `Η υπόθεση της όπερας αφορά τον μοιραίο έρωτα της δεκαπεντάχρονης γκέισας Τσο-Τσο-Σαν για τον Πίνκερτον, υποπλοίαρχο του Ναυτικού των Ηνωμένων Πολιτειών της Αμερικής. Ύστερα από τρία χρόνια απουσίας, ο αξιωματικός επιστρέφει με την Αμερικανίδα σύζυγό του στην Ιαπωνία, μαθαίνοντας ότι έχει αποκτήσει γιο από την Μπαττερφλάι. Εκείνη δέχεται να παραδώσει το παιδί μονάχα στον ίδιο τον Πίνκερτον και στη συνέχεια αυτοκτονεί. Διάσημη για τις υπέροχες άριες, την πρόδηλα μελωδική μουσική και τη δραματική θεατρικότητά της, η Μαντάμα Μπαττερφλάι συγκινεί διαχρονικά και προκαλεί έντονα συναισθήματα. Ο Πουτσίνι δεν διστάζει να τη χαρακτηρίσει ως την πιο αγαπημένη του όπερα, ενώ με τις μετέπειτα τροποποιήσεις ανάγει την αλαβάστρινη μορφή της ηρωίδας του σε σύμβολο ανεξάντλητης υπομονής και αιώνιας, σταθερής αγάπης. Τη σκηνοθεσία, τα σκηνικά και τα κοστούμια της παραγωγής, η οποία πρωτοπαρουσιάστηκε το 2013 στο Ηρώδειο, και τώρα αναβιώνει σε μια νέα εκδοχή για τη σκηνή της Αίθουσας Σταύρος Νιάρχος, υπογράφει ο διάσημος Αργεντινός σκηνοθέτης Ούγκο ντε Άνα. Πρόκειται για μια εντυπωσιακή παραγωγή με παραδοσιακά γιαπωνέζικα κοστούμια, ενώ τα σκηνικά και οι προβολές εικονοποιούν με εντυπωσιακό τρόπο τη χώρα του ανατέλλοντος ηλίου από τη μια και τον ψυχισμό της ηρωίδας από την άλλη. Τις προβολές υπογράφει ο Σέρτζιο Μετάλλι, ενώ τους φωτισμούς ο Βαλέριο Αλφιέρι.`,
    dates: [
      "2021-10-14",
      "2021-10-16",
      "2021-10-18",
      "2021-10-20",
      "2021-10-23",
      "2021-10-25",
      "2021-10-30",
      "2021-11-01",
      "2021-11-08",
      "2021-11-15",
    ],
    crew: {
      conductor: "Λουκάς Καρυτινός",
      director: "Ούγκο ντε Άνα",
      "video projection designer": "Σέρτζιο Μετάλλι - Ideogamma SRL",
      sets: "Ούγκο ντε Άνα",
      costumes: "Ούγκο ντε Άνα",
      "lighting designer": "Βαλέριο Αλφιέρι",
      "chorus master": "Αγαθάγγελος Γεωργακάτος",
    },
    roles: {
      "Τσο-Τσο-Σαν": {
        "Ερμονέλα Γιάχο": [
          "2021-10-14",
          "2021-10-16",
          "2021-10-25",
          "2021-10-25",
        ],
        "Τσέλια Κοστέα": ["2021-11-15"],
        "Κριστίνε Οπολάις": [
          "2021-10-18",
          "2021-10-23",
          "2021-10-30",
          "2021-11-01",
          "2021-11-11",
        ],
      },
      Σουτζούκι: {
        "Χρυσάνθη Σπιτάδη": [
          "2021-10-14",
          "2021-10-16",
          "2021-10-18",
          "2021-10-25",
          "2021-10-23",
          "2021-10-25",
          "2021-10-30",
          ,
          "2021-11-01",
          "2021-11-08",
          "2021-11-15",
        ],
      },
      "Κέιτ Πίνκερτον": {
        "Βιολέττα Λούστα": [
          "2021-10-16",
          "2021-10-18",
          "2021-10-25",
          "2021-10-23",
          "2021-10-25",
        ],
        "Διαμάντη Κριτσωτάκη": [
          "2021-10-14",
          "2021-10-30",
          "2021-11-01",
          "2021-11-08",
          "2021-11-15",
        ],
      },
      "Μ.Φ.Πίνκερτον": {
        "Τζανλούκα Τερρανόβα": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-23",
          "2021-10-25",
        ],
        "Δημήτρης Πακσόγλου": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-30",
          "2021-11-01",
          "2021-11-08",
          "2021-11-15",
        ],
      },
      Σάρπλες: {
        "Διονύσης Σούρμπης": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-23",
          "2021-10-25",
        ],
        "Νίκος Κοτενίδης": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-30",
          "2021-11-01",
          "2021-11-08",
          "2021-11-15",
        ],
      },
      Γκόρο: {
        "Νίκος Στεφάνου": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-23",
          "2021-10-25",
        ],
        "Γιάννης Καλύβας": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-30",
          "2021-11-01",
          "2021-11-08",
          "2021-11-15",
        ],
      },
      "Πρίγκιπας Γιαμαντόρι": {
        "Μάριος Σαραντίδης": [],
      },
      Μπόνζο: {
        "Γιάννης Γιαννίσης": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-23",
          "2021-10-25",
          "2021-11-01",
        ],
        "Δημήτρης Κασιούμης": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-30",
          "2021-11-08",
          "2021-11-15",
        ],
      },
      Γιακουζιντέ: {
        "Πέτρος Σαλάτας": [],
      },
      "Αυτοκρατορικός Επίτροπος": {
        "Διονύσης Τσαντίνης": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-25",
          "2021-11-01",
          "2021-11-15",
        ],
        "Γιώργος Παπαδημητρίου": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-23",
          "2021-10-30",
          "2021-11-08",
        ],
      },
      Ληξίαρχος: {
        "Θεόδωρος Αϊβαλιώτης": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-25",
          "2021-11-01",
          "2021-11-15",
        ],
        "Θεόδωρος Μωραΐτης": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-23",
          "2021-10-30",
          "2021-11-08",
        ],
      },
      "Μητέρα της Τσο - Τσο - Σαν": {
        "Αμαλία Αυλωνίτη": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-25",
          "2021-11-01",
          "2021-11-15",
        ],
        "Ζωή Απειρανθίτου": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-23",
          "2021-10-30",
          "2021-11-08",
        ],
      },
      Θεία: {
        "Βάγια Κωφού": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-25",
          "2021-11-01",
          "2021-11-15",
        ],
        "Ελισάβετ Κλονόφσκαγια": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-23",
          "2021-10-30",
          "2021-11-08",
        ],
      },
      Ξαδέρφη: {
        "Φωτεινή Χατζιδάκη": [
          "2021-10-16",
          "2021-10-25",
          "2021-10-25",
          "2021-11-01",
          "2021-11-15",
        ],
        "Θέη Σταύρου": [
          "2021-10-14",
          "2021-10-18",
          "2021-10-23",
          "2021-10-30",
          "2021-11-08",
        ],
      },
    },
  },
};

/*  */

var title1 = document.querySelector(".card-title1");

var div1 = document.createElement("div");

div1.innerHTML = shows["Don Giovanni"]["name"];

title1.append(div1);

var composer = document.querySelector(".card-text1");

var div1 = document.createElement("div");
div1.classList.add("role-name2");
div1.innerHTML = shows["Don Giovanni"]["composer"];
div1.innerHTML += "<span class='role'>" + "Σύνθεση" + "</span>";

composer.append(div1);

var libreto = document.querySelector(".card-text1");

var div1 = document.createElement("div");
div1.classList.add("role-name2");
div1.innerHTML = shows["Don Giovanni"]["libretto"];
div1.innerHTML += "<span class='role'>" + "Λιμπρέτο" + "</span>";

libreto.append(div1);

var prod = document.querySelector(".card-text1");

var div1 = document.createElement("div");
div1.classList.add("production");
div1.innerHTML = shows["Don Giovanni"]["production"];

prod.append(div1);

/*madam butterfly*/

var title2 = document.querySelector(".card-title2");

var div1 = document.createElement("div");

div1.innerHTML = shows["Madame Butterfly"]["name"];

title2.append(div1);

var composer2 = document.querySelector(".card-text2");

var div1 = document.createElement("div");
div1.classList.add("role-name2");
div1.innerHTML = shows["Madame Butterfly"]["composer"];
div1.innerHTML += "<span class='role'>" + "Σύνθεση" + "</span>";

composer2.append(div1);

var libreto2 = document.querySelector(".card-text2");

var div1 = document.createElement("div");
div1.classList.add("role-name2");
div1.innerHTML = shows["Madame Butterfly"]["libretto"];
div1.innerHTML += "<span class='role'>" + "Λιμπρέτο" + "</span>";

libreto2.append(div1);

var prod2 = document.querySelector(".card-text2");

var div1 = document.createElement("div");
div1.classList.add("production");
div1.innerHTML = shows["Madame Butterfly"]["production"];

prod2.append(div1);

/* FIGARO */

var title3 = document.querySelector(".card-title3");

var div1 = document.createElement("div");

div1.innerHTML = shows["The Marriage of Figaro"]["name"];

title3.append(div1);

var composer3 = document.querySelector(".card-text3");

var div1 = document.createElement("div");
div1.classList.add("role-name2");
div1.innerHTML = shows["The Marriage of Figaro"]["composer"];
div1.innerHTML += "<span class='role'>" + "Σύνθεση" + "</span>";

composer3.append(div1);

var libreto3 = document.querySelector(".card-text3");

var div1 = document.createElement("div");
div1.classList.add("role-name2");
div1.innerHTML = shows["The Marriage of Figaro"]["libretto"];
div1.innerHTML += "<span class='role'>" + "Λιμπρέτο" + "</span>";

libreto3.append(div1);

var prod3 = document.querySelector(".card-text3");

var div1 = document.createElement("div");
div1.classList.add("production");
div1.innerHTML = shows["The Marriage of Figaro"]["production"];

prod3.append(div1);

/* CALENDAR */
var dates_arr = [];
for (i in shows) {
  for (j in shows[i]["dates"]) {
    dates_arr.push([shows[i]["dates"][j], i]);
  }
}

dates_arr.sort();

event_right = document.querySelector(".eventright");
event_left = document.querySelector(".eventleft");
buttonkratisis = document.querySelector(".buttonkratisis");
//console.log("main div is ", main_div);

var months = [
  "empty",
  "Ιανουαρίου",
  "Φεβρουαρίου",
  "Μαρτίου",
  "Απριλίου",
  "Μαϊου",
  "Ιουνίου",
  "Ιουλίου",
  "Αυγούστου",
  "Σεπτεμβρίου",
  "Οκτωβρίου",
  "Νοεμβρίου",
  "Δεκεμβρίου",
];

function convert_date(string_date) {
  year = string_date.split("-")[0];
  month = string_date.split("-")[1];
  day = string_date.split("-")[2];
  month_name = months[parseInt(month)];
  new_string = day + "\n" + month_name + "\n" + year;
  return new_string;
}

var id = 0;
for (i in dates_arr) {
  var div1 = document.createElement("div");
  var div2 = document.createElement("div");
  var button1 = document.createElement("button");
  button1.id = id;
  id += 1;
  div1.classList.add("event1");
  div2.classList.add("event2");
  button1.classList.add("button2");
  button1.innerHTML = "Κάντε Κράτηση";
  let show_name = dates_arr[i][1];
  let show_date = dates_arr[i][0];
  div1.innerHTML = convert_date(show_date); //+ "&nbsp&nbsp&nbsp&nbsp";
  div2.innerHTML = show_name;

  event_right.append(div1);
  event_left.append(div2);
  buttonkratisis.append(button1);
}

collection = document.getElementsByClassName("button2");

//console.log(div_collection);

for (let item of collection) {
  item.onclick = function () {
    show_date = dates_arr[parseInt(item.id)][0];
    show_name = dates_arr[parseInt(item.id)][1];

    window.location.href = "reservationnew.html#" + show_name + "&" + show_date;
  };
}
