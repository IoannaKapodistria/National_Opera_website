...
function pageLoad() {
  var url = window.location.hash.slice(1); //otan erxetai apo tis krathseis einai url= ""
  console.log(url);
  var show_name = url.split("&")[0].replaceAll("%20", " "); // otan erxetai apo tis krathseis einai show_name =""
  var show_date = url.split("&")[1]; // otan erxetai apo tis krathseis einai show_date =""
  console.log(show_name);
  console.log(show_date);
  document.getElementById("parastasi").value = show_name;

  if (show_name !== "") {
    //auto trexei mono otan erxetai apo to programm.html

    showOptions(show_name);
    document.getElementById("date1").value = show_date;
  }
}

on_change_show_options();

function on_change_show_options() {
  var parastasi = document.getElementById("parastasi");
  console.log("parastasi is : ", parastasi);

  parastasi.onchange = function () {
    var selectedparastasi = parastasi.options[parastasi.selectedIndex].value;
    //console.log("selected parastasi is", selectedparastasi);
    showOptions(selectedparastasi);
  };
  //console.log("selected parastasi is", selectedparastasi);
}

function showOptions(name_of_show) {
  var dates = shows[name_of_show]["dates"];
  //console.log(dates[2], name_of_show);
  var dates_select = document.getElementById("date1");
  while (dates_select.length > 0) {
    dates_select.remove(0);
  }

  for (i in dates) {
    var newdate_option = document.createElement("option");
    newdate_option.text = dates[i];
    dates_select.add(newdate_option);
  }
}
...
let tilefono = document.querySelector("#phonenumber");
//

var submit_button = document.getElementsByClassName("epomeno")[0];
submit_button.onclick = function () {
  //console.log(111);
  if (!username1.checkValidity()) {
    alert("Μη έγκυρη μορφή ονόματος");
  }

  if (!surename.checkValidity()) {
    alert("Μη έγκυρη μορφή επωνύμου");
  }
  ...
  if (!tilefono.checkValidity()) {
    alert("Μη έγκυρη μορφή τηλεφώνου");
  }
  if (forma.checkValidity()) {
    window.location.href = "canvasgrid.html";
  }
};
